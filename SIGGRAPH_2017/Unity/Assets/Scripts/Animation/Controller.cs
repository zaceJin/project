...
public class Controller {
	...
	public Vector3 QueryMove(int i =0) {
		...
		if(InputHandler.GetKey(Left) || i == 3) {
			move.x -= 1.0f;
		}
		if(InputHandler.GetKey(Right) || i == 4) {
			move.x += 1.0f;
		}
		return move;
	}
	...
	public float QueryTurn(int i = 0) {
		float turn = 0f;
		if(InputHandler.GetKey(TurnLeft)||i ==1) {
			turn -= 0.08f;
		}
		if(InputHandler.GetKey(TurnRight)||i==2) {
			turn += 0.08f;
		}
		return turn;
	}
